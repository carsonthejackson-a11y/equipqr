...
// RLS policies from migration 0022. The endpoint `secret` is returned to the
...
export async function deleteWebhookEndpoint(endpointId: string) {
  ...
  const supabase = await createClient();
  // Deliveries cascade with the endpoint (migration 0022).
...
/** Puts a `failed` delivery back in the queue with a fresh attempt budget (RPC from migration 0022). */
