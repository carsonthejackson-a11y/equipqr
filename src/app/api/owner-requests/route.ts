...
export async function POST(request: Request) {
  ...
  const ownerNotified = await sendOwnerNotificationEmail(result, body, description, priorityLabel);
  ...
  return NextResponse.json({
    ...
    // Whether the owner's own back-office actually got the notification
    // email — the confirmation screen only claims "{company} was notified"
    // when this is true (C1-30, Q-12).
    ownerNotified,
  });
}
