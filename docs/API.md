...
Rows also carry `custom_fields`: an object keyed by the custom field `key` defined under
Settings → Custom fields, e.g. `{ "filter_size": "20x25", "has_drain_pan": true }`. Keys without
a value are absent.
