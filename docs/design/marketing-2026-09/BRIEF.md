...
- `src/lib/features.ts`: `FEATURES.batchQr` (env-flag, default on) gates the "Pre-printed batch QR sticker orders" compare row in `pricing/audience-tabs.tsx`. (The `FEATURES.ownerAccounts` flag this once mentioned was removed as dead code, C1-42 — owner accounts are unconditional now.)
